...
class TurtleFollower(Node):
    def __init__(self):
        ...
        self.chase_timer = self.create_timer(0.1, self.chase_closest_turtle)
    ...
    def pose_callback(self, data):
        ...
        for turtle_name, (x2, y2, _) in self.turtle_positions.items():
            if turtle_name in self.unhit_turtles and self.turtles_collide(self.x1, self.y1, x2, y2):
                ...
                follow_timer = self.create_timer(0.1, self.create_follow_callback(turtle_name))
    ...
    def create_follow_callback(self, turtle_name):
        def follow_callback():
            ...
            distance = math.sqrt((self.y1 - y2)**2 + (self.x1 - x2)**2)
            angle_to_goal = math.atan2(self.y1 - y2, self.x1 - x2)
